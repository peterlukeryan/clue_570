...
class bluffPlayer:
    def __init__(self, player_id=None, player_map=None, bluff_probability=0.5): #change that to whatever
        self.player_map = player_map if player_map is not None else {}  # player map initializes as dictionary
        self.player_id = player_id  # Now using ids
        self.suspects = []
        self.bluff_probability = bluff_probability  # Probability of bluffing (50% chance)
...
#i genuinely dont remember what was the point of this 
    ...
    def make_accusation(self, players, turn, num_players):
        accusation_index = 1
        while accusation_index <= num_players:
            # Decide whether to bluff or not based on the bluff probability
            if random.random() < self.bluff_probability:
                print("Bluffing Bluffing Bluffing Bluffing Bluffing")
                # Select one random card from the player's hand to bluff
                bluff_card = random.choice(self.player_map[self.player_id])
                ...
                # If not bluffing, select one card from each suspect list
                print("NOT Bluffing NOT Bluffing NOT Bluffing NOT Bluffing")
                char_suspect = random.choice(self.suspects[0])
                weapon_suspect = random.choice(self.suspects[1])
                room_suspect = random.choice(self.suspects[2])
                
                # Create the accusation cards list without any bluff card
                accusation_cards = [char_suspect, weapon_suspect, room_suspect]
